...
// void keyboard_detection_row();
// void keyboard_detection_col(uint n);
...
dfloat d_add_dd(dfloat d1, dfloat d2) {
    ...
    if (d_result.f_mid >= 65535) d_result.f_mid = 0;
    if (d_result.f_mid <= -65534) {
        d_result.f_mid = 0;
        d_result.f_high = d_result.f_high + 1;
    }
    if (d_result.f_low >= 65535) d_result.f_low = 0;
...
dfloat d_div_dd(dfloat d1, dfloat d2) {
	...
	if (d2.f_mid >= 65535) d2.f_mid = 0;
    if (d2.f_mid <= -65534) {
        d2.f_mid = 0;
        d2.f_high = d2.f_high + 1;
    }
    f_high = d1.f_high / (d2.f_high + d2.f_mid + d2.f_low);
    // f_high_tmp = (int)f_high;
    // if (f_high_tmp == 0) {
    //     bits_10 = 1;
    // } else{
    // bits_10 = log10(abs(f_high_tmp)) + 1; //计算整数位数
    // }
    // switch (bits_10) {
    // {
    // case 1:
...
{
    OpeStack *p = (OpeStack *)malloc(sizeof(OpeStack)); //?????
    if (p == NULL)                                      //????
        return 0;
    p->bottom = p->top = 0; //????
    return p;
...
int stack_calculation(NumStack* n, OpeStack* o) {
    ...
    case divide_sign:
        if (fabs(n2.f_high) < 1e-8){
            state_cal = 2;//在精度为1e-8条件下判断除数是否为0，为0输出计算数学错误
            return -1; 
        }
...
int numinstack(uint num, NumStack* p, uint dot_occupied) {
    ...
    if (dot_occupied == 0) {
        ...
        {
            num_ans = d_mul_df(num_ans, 10);
        }
        num_ans = d_add_df(num_ans, (float)num);
        for (i = 0; i < dot_occupied; i++)
        {
            num_ans = d_div_df(num_ans, 10);
        }
    }
...
int sqrt_cal(NumStack *n, int* sqrt_flag)
{
    ...
    for(i = 0; i < n->top; i++)
    {
        if (sqrt_flag[i] == 1)
        {
            ...
            f_high_tmp = 10000 * n->data_s[i].f_high;
            f_high_tmp_int = (int)f_high_tmp;
            n->data_s[i].f_mid = f_high_tmp - f_high_tmp_int;
...
//void ldouble2char(ldouble f, uchar *uchar_array)
//{
//    int i = 0;
//    uchar tmp[16];
//    int pointer;
//    int int_part = (int)f;
//    ldouble ldouble_part = f - int_part;
//    if (f < 0)
//    {
//        uchar_array[i++] = '-';
//        f = -f;
...
int Key_Scan()
{
    ...
    if (button != 0x0f) //有键按下
    {
        ...
        delay(10); //消抖
        button = P4 & 0x0f;
        if (button != 0x0f) //第一行 在上升沿写入一位低电平，开始逐行检测 若P4再次变0，说明按下的键在该行，然后再根据之前存的BUTTON值求键值
        {
            send_data(0xFE);
            temp = P4 & 0x0f;

            if (temp != 0x0f) //若P4再次变1，说明按下的键在该行，然后再根据之前存的BUUTON值求键值
            {
                while (P4 != 0xFF)
                    ;
                switch (button)
...
void main()
{
    ...
    while (1)
    {
        //        chrt_disp_first(input_char, length_input);
        //        cgram_wr(0x00, 0x00);
        //        cgram_disp();
        switch (state_now)
        {
